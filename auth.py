...
# In the create_user function, make sure the email is stored correctly
def create_user(username, password, email):
    """Create a new user"""
    users = load_users()
    ...
    users[username] = {
        'password': hash_password(password),
        'email': email,  # Already storing email, make sure it's used in the UI
        'created_at': datetime.now().isoformat(),
        'last_login': None,
        'history': []
    }
...
# Add this new function to get user email
def get_user_email(username):
    """Get the email address of a user"""
    users = load_users()
    if username in users and 'email' in users[username]:
